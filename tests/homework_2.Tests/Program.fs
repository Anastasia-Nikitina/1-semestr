      ...
      testList "Homework_2 Tests"
        [testCase "Если x=0, результат равен 0" <| fun _ ->
         ...
         testCase "Если x=0, результат равен 0" <| fun _ ->
          ...
          Expect.equal subject 0 "0 в любой степени(кроме нулевой) дает 0. "
         ...
         testCase "Любое отрицательное число всегда меньше нуля" <| fun _ ->
           ...
           Expect.equal subject [|4|] "-1-отрицательное число, значит -1<0"
         ...
         testCase "Ноль находится в промежутке между -1 и 1" <| fun _ ->
             ...
             Expect.equal subject [|2; 4|] "Результат должен выводить массив с номерами элементов-нулей"
