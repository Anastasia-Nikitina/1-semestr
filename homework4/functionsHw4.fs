...
let arrayIntoString (array: array<int>) =
    let stringArr: string [] = Array.zeroCreate array.Length
...
let bubbleSortOfList (list: int list) = 
    let rec bubble (list1: int list) =
      match list1 with
      | [] -> []
      | head :: [] -> [head]
      | head1 :: head2 :: tail ->
          if head1 > head2 then head2 :: bubble(head1 :: tail)
          else head1 :: bubble(head2 :: tail)
    let rec go (list2: int list) k =
        if k <> list2.Length then
            go(bubble list2) (k+1)
        else list2
    go list 0
   
let quickSortOfArray (a: array<int>) =
    let rec sort (arr: int [], first: int, last: int) =
          let mutable i = first
          let mutable j = last
          ...
          while  i <= j do
              ...
              then
                  ...
                  i <- i + 1
                  j <- j - 1
          if i < last then sort (arr, i, last)
          if first < j then sort (arr, first, j)
    if a = [||] then a
    else
      sort (a, 0, a.Length - 1)
      a
...
let packing32BitNumber (x: int32, y: int32) =
    if y >= 0 then  (int64 x <<< 32) +  int64 y
...
let packing16BitNumber (a: int16, b: int16, c: int16, d: int16) =
    let pack16to32 (x: int16, y: int16) =
        if y >= 0s then  (int32 x <<< 16) +  int32 y
